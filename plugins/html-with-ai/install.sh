#!/bin/sh
# html-with-ai 설치 (Mac/Linux). 파이썬 3가 없으면 설치까지 한다. Windows 는 install.ps1.
set -e
...
# 2) 에이전트별 스킬 위치에 링크
mkdir -p "$HOME/.claude/skills" "$HOME/.config/html-with-ai"
[ "$HERE" = "$HOME/.claude/skills/html-with-ai" ] || ln -sfn "$HERE" "$HOME/.claude/skills/html-with-ai"
echo "Claude Code : ~/.claude/skills/html-with-ai"
