#!/usr/bin/env python3
"""html-with-ai 훅 — 에이전트가 HTML 을 만들면 어떤 도구로 만들었든 편집 화면이 뜨게 한다.
...
처리한 문서는 ~/.config/html-with-ai/state.json 에 세션별로 기록해 같은 문서를 두 번 띄우지 않는다.
"""
import sys, os, re, json, pathlib, subprocess, socket, time, urllib.request
HERE = pathlib.Path(__file__).resolve().parent
CFG_DIR = pathlib.Path.home() / ".config" / "html-with-ai"
...
HTML_RE = re.compile(r"""(?<![\w./-])((?:~|\.{1,2})?/?[^\s'"`;|&<>()]*?\.html?)(?=$|[\s'"`;|&<>)])""", re.I)
...
def recent_htmls(cwd, since, handled):
    ...
    for root, dirs, files in os.walk(cwd or "."):
        ...
        for f in files:
            if f.lower().endswith((".html", ".htm")) and not f.endswith(".bak"):
...
def instruct(event, doc, cfg, agent, sid):
    ...
    lead = "[html-with-ai]" + (" (턴 종료 안전망 — 저장 훅이 놓친 HTML)" if event == "Stop" else "")
...
def main():
    ...
    if not doc.exists() or "_review_" in doc.name or doc.name.endswith(".bak"): sys.exit(0)
