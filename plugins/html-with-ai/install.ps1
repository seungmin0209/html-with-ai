# html-with-ai 설치 (Windows PowerShell). 파이썬 3가 없으면 winget 으로 설치까지 한다.
...
$Dest = Join-Path $env:USERPROFILE ".claude\skills\html-with-ai"
if ((Resolve-Path $Here).Path -ne $Dest) {
  New-Item -ItemType Directory -Force -Path $Dest | Out-Null
  Copy-Item -Path (Join-Path $Here "*") -Destination $Dest -Recurse -Force
}
New-Item -ItemType Directory -Force -Path (Join-Path $env:USERPROFILE ".config\html-with-ai") | Out-Null
